...
var WF_COMMIT_PROP_MODE_Automatic string = "Automatic"
...
func insertWorkflowStep(c *cobra.Command, db *sqlx.DB) error {
	return ExecInsertCommand[DPRProjectionConfigStep](c, db, "VI.Projector.Projection.ProjectionStep", newWorkflowStepCmd)
}

func newWorkflowStepCmd(
	...
	// is this a data import?
	isImport, _ := c.Flags().GetBool("source-is-authoritative")

	// give the new step a reasonable sequence num
	currentCount, err := CountWorkflowSteps(db, wfId)
	if err != nil {
		return nil, err
	}

	return newWorkflowStep(id, objectKey, name, clrId, wfId, currentCount+1,
		leftConnectionId, rightConnectionId, mapId, matchSetsId,
		isImport)
}
...
func newWorkflowStep(
	id string, objectKey string, name string,
	clrId string,
	wfId string, sequence int,
	leftConnectionId string, rightConnectionId string, mapId string, matchSetsId string,
	isImport bool,
) (*DPRProjectionConfigStep, error) {

	description := "step generated by SPEd"
	...
	t := DPRProjectionConfigStep{
		...
		IsImport:             isImport,
		CommitPropertiesMode: &WF_COMMIT_PROP_MODE_Automatic,
		Sequence:             sequence,
	}
	if isImport {
		t.ProjectionDirection = WORKFLOW_DIRECTION_Left
	} else {
		t.ProjectionDirection = WORKFLOW_DIRECTION_Inherite
	}
